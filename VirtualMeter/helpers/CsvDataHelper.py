...
def GetData(filepath):
    """
    Loads data from a CSV file, filters out rows with invalid entries, and returns the processed data.
    The function reads in a CSV file containing energy consumption data, parses it into a NumPy array, 
    and filters out rows where the third column (index 2) contains invalid data (e.g., "?"). This function 
    assumes that the CSV file uses a semicolon (';') as the delimiter and skips the first row (header).
    :param filepath: The path to the CSV file containing the energy consumption data.
    :return: A NumPy array containing the processed data from the CSV file, with rows containing valid entries.
    """
...
def ExtractPastVirtualMeterCsvData(filepath, start_time):
    ...
    :param start_time: A datetime object representing the exact end time point for the historical data being extracted (inclusive)
    :return: A tuple containing:
             - yesterday_daily_data: A list of tuples (time, intensity, voltage) for the day before the start date
             - daily_data: A list of tuples (time, intensity, voltage) for the start date up to the and including the start time
    ...
    overall_data = []  # List to store summaries of all days except the start day
    ...
    for row in data:
        ...
        # If the date is the day before the start date
        ...
        # If the date is the same date as the start date
        elif date == start_time.date():
            if time <= start_time:
                daily_data.append((time, intensity, voltage))

        # Date is before start date and day before start date        
        else:
            if time < start_time:
                if date not in historical_daily_data_dict:
                    historical_daily_data_dict[date] = []
    ...
    # Calculate the daily summary for all dates in the historical_daily_data_dict
...
# This is an old function that is no longer in use (made redundant by the more robust functions above)
